...
RSpec.describe Contact, type: :model do
  ...
  context "validations" do
    ...
    it "email format validation" do
      ...
      expect(new_contact.errors.messages[:email].first).to eq("Email invalid")
    end
    ...
    it "phone number format validation" do
      ...
      expect(new_contact.errors.messages[:phone_number].first).to eq("Phone number invalid")
    end
    ...
    it "custom email validation" do
      ...
      expect(new_contact2.errors.messages[:email].first).to eq("Repeated email")
    ...
    it "custom birth date validation" do
      ...
      expect(new_contact.errors.messages[:birth_date].first).to eq("Birth date invalid")
    end
    ...
    it "custom credit card validation" do
      ...
      expect(new_contact.errors.messages[:credit_card].first).to eq("Credit card invalid")
    end
